...
import { LogOut, LayoutDashboard, Users, Calendar, Settings, Menu, X, Bell, HeartHandshake, Building } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col md:flex-row">
      ...
      `}>
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto mt-16 md:mt-0">
          <Link href="/admin/dashboard" onClick={() => setIsMobileMenuOpen(false)} className="group flex items-center gap-3 px-3 py-2 text-indigo-600 bg-indigo-50 dark:bg-indigo-500/10 rounded-md font-medium transition-all duration-200 hover:translate-x-1 hover:bg-indigo-100 dark:hover:bg-indigo-500/20">
            <LayoutDashboard className="w-5 h-5 text-indigo-600 transition-transform group-hover:scale-110" />
            Overview
          </Link>
          <Link href="/admin/ngos" onClick={() => setIsMobileMenuOpen(false)} className="group flex items-center gap-3 px-3 py-2 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800/80 rounded-md font-medium transition-all duration-200 hover:translate-x-1">
            <Building className="w-5 h-5 transition-transform group-hover:scale-110 group-hover:text-indigo-500" />
            Manage NGOs
          </Link>
          <Link href="/admin/volunteers" onClick={() => setIsMobileMenuOpen(false)} className="group flex items-center gap-3 px-3 py-2 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800/80 rounded-md font-medium transition-all duration-200 hover:translate-x-1">
            <HeartHandshake className="w-5 h-5 transition-transform group-hover:scale-110 group-hover:text-indigo-500" />
            Manage Volunteers
          </Link>
          <Link href="/admin/events" onClick={() => setIsMobileMenuOpen(false)} className="group flex items-center gap-3 px-3 py-2 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800/80 rounded-md font-medium transition-all duration-200 hover:translate-x-1">
            <Calendar className="w-5 h-5 transition-transform group-hover:scale-110 group-hover:text-indigo-500" />
            All Events
          </Link>
        </nav>
